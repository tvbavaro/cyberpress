<template>
  <ul class="list__column" v-if="isDropDown">
    <li class="list__item list__item-header" @click.stop="isOpen = !isOpen">
      ...
      <svg
        class="list__arrow"
        :class="{ list__arrow_close: !isOpen }"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
      ...
      <li class="list__item" v-for="item in list" :key="item">
        <template v-if="hasCheckbox">
          <input
            ...
            @click.stop
            @change="executableFunction($event.target.checked, item)"
            @keydown.enter="
              $event.target.checked = !$event.target.checked;
              executableFunction($event.target.checked, item);
            "
            type="checkbox"
            :id="item"
            :name="item"
          ...
          />
          <label :for="item"
            ><span class="list__text">{{ item }}</span></label
          >
        </template>
        <template v-else>
          <span
            class="list__text"
            @click="executableFunction(item)"
            @keydown.enter="executableFunction(item)"
            tabindex="0"
            >{{ item }}</span
          >
        </template>
      ...
      <li class="list__item" v-for="item in list" :key="item">
        <template v-if="hasCheckbox">
          <input
            ...
            :checked="isChecked(item)"
          />
          <label class="list__label" :for="item"
            ><span class="list__text list__text_checkbox">{{
              item
            }}</span></label
          >
        </template>
...
<script>
import { isString, isArray, isBoolean, isFunction } from "@helpers/validators";
